import { 
  IsString, 
  IsNotEmpty, 
  Matches, 
  Length
} from 'class-validator';

/**
 * DTO (Data Transfer Object) para la creaciAn de un nuevo rol.
 *
 * Este objeto define y valida los datos que se esperan al momento de
 * recibir una solicitud para crear un rol.
 */
export class CreateRoleDto {
  /**
   * DescripciAn del rol (por ejemplo: "admin", "user").
   *
   * @type {string}
   * @validation
   * - Debe ser una cadena (`string`).
   * - No puede estar vacAa.
   */
  @IsString()
  @IsNotEmpty({ message: 'La descripciAn no puede estar vacAa' })
  @Matches(/^[a-zA-Z\s]+$/, {
    message: 'La descripciAn solo debe contener letras y espacios',
  })
  @Length(3, 30, {
    message: 'La descripciAn debe tener entre 3 y 30 caracteres',
  })
  description: string;
  
}


...
export class UpdateUserRoleUseCase {
  ...
  async execute(id: string, roleId: string, requesterRole: RoleType) {
    if (requesterRole !== RoleType.ADMIN) {
      throw new UnauthorizedException('No autorizado para cambiar roles de usuario');
    }
    ...
    if (target.role.description === RoleType.ADMIN) {
      throw new UnauthorizedException('No puedes cambiar el rol de un administrador');
    }
    ...
    if (nextRole.description === RoleType.ADMIN) {
      throw new UnauthorizedException('No puedes asignar rol administrador');
    }

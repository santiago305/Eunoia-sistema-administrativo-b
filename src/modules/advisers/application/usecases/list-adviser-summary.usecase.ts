...
export class ListAdviserSummaryUsecase {
  ...
  async execute(input: { page?: number; limit?: number; q?: string; filters?: string | AdviserSearchRule[]; requestedBy?: string }) {
    ...
    builder.select('a.user_id', 'id')
      ...
      .orderBy('u.name', 'ASC');
    const numericExpressions: Record<string, string> = { assignedOrders: 'COUNT(DISTINCT o.id)', soldTotal: 'COALESCE(SUM(o.total), 0)', collectedTotal: '(SELECT COALESCE(SUM(sp.amount), 0) FROM sale_payments sp INNER JOIN sale_orders so2 ON so2.id = sp.sale_order_id AND so2.assigned_by = a.user_id AND so2.is_active = true)' };
    snapshot.filters.forEach((rule, index) => {

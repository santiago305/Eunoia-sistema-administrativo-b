...
export class SaveFullWorkflowUseCase {
  ...
  async execute(input: SaveFullWorkflowInput) {
    return this.uow.runInTransaction(async (tx) => {
      const current = input.workflowId
        ? await this.workflowRepo.findDetailedById(input.workflowId, tx)
        : null;
  ...
  private async buildAggregate(
    input: SaveFullWorkflowInput,
    current: WorkflowAggregate | null,
    tx: any,
    ...
    this.assertUnique(
      input.states.map((state) => state.clientId),
      'clientId de estado',
    );
    this.assertUnique(
      input.states.map((state) => state.saleOrderStateId ?? state.code ?? ''),
      'estado global',
    );
    this.assertUnique(
      input.transitions.map((transition) => transition.clientId),
      'clientId de transicion',
    );
    this.assertUnique(
      input.transitions.map((transition) => transition.code.trim()),
      'code de transicion',
    );
    if (
      input.transitions.filter(
        (transition) =>
          (transition.purpose ?? TRANSITION_PURPOSES.STANDARD) ===
          TRANSITION_PURPOSES.CANCEL,
      ).length > 1
    ) {
      throw new BadRequestException(
    ...
    const automaticRoutePriorities = input.transitions
      .filter(
        (transition) =>
      ...
      .map(
        (transition) =>
          `${transition.fromStateRef}:${transition.priority ?? 0}`,
      );
    if (
      new Set(automaticRoutePriorities).size !== automaticRoutePriorities.length
    ) {
      throw new BadRequestException(
    ...
    const existingStateIds = new Set(
      current?.states.map((state) => state.id) ?? [],
    );
    ...
    for (const state of input.states) {
      if (state.id && !existingStateIds.has(state.id)) {
        throw new BadRequestException(
          `El estado ${state.id} no pertenece al workflow`,
        );
      }
    }
    for (const transition of input.transitions) {
      if (transition.id && !existingTransitionIds.has(transition.id)) {
        throw new BadRequestException(
          `La transicion ${transition.id} no pertenece al workflow`,
        );
      }
    }
    ...
    const requestedGlobalIds = input.states
      .map((state) => state.saleOrderStateId)
      .filter(Boolean) as string[];
    const globalStates = new Map(
      (
        await Promise.all(
          requestedGlobalIds.map((id) =>
            this.saleOrderStatesRepo.findById(id, tx),
          ),
        )
      )
        .filter(Boolean)
    ...
    if (
      states.filter((state) => state.isInitial && state.isActive).length !== 1
    ) {
      throw new BadRequestException(
        'El workflow requiere exactamente un estado inicial activo',
      );
    }
    if (!states.some((state) => state.isFinal && state.isActive)) {
      throw new BadRequestException(
        'El workflow requiere al menos un estado final activo',
      );
    }
    ...
    const transitions = input.transitions.map((transition) => {
      ...
      if (purpose === TRANSITION_PURPOSES.CANCEL && !isGlobal) {
        throw new BadRequestException(
          'La transicion de cancelacion debe ser global',
        );
      }

      const fromState = transition.fromStateRef
        ? stateReferences.get(transition.fromStateRef)
        : null;
      const toState = transition.toStateRef
        ? stateReferences.get(transition.toStateRef)
        : null;
      const elseToState = transition.elseToStateRef
        ? stateReferences.get(transition.elseToStateRef)
        : null;
      if (!isGlobal && !fromState) {
        throw new BadRequestException(
          `Transicion ${transition.code} referencia estados inexistentes`,
        );
      }
      if (effect === TRANSITION_EFFECTS.MOVE_STATE) {
        ...
        if (purpose === TRANSITION_PURPOSES.CANCEL && toState.isFinal) {
          throw new BadRequestException(
            'El estado destino de cancelacion no puede ser final',
          );
        }
      }
      if (effect === TRANSITION_EFFECTS.RUN_ACTIONS && transition.toStateRef) {
        throw new BadRequestException(
          `Transicion de acciones ${transition.code} no debe tener estado destino`,
        );
      }
      if (isGlobal && transition.fromStateRef) {
        throw new BadRequestException(
          `Transicion global ${transition.code} no debe tener estado origen`,
        );
      }
      if (transition.autoTrigger && !transition.conditions?.length) {
        throw new BadRequestException(
          `Transicion automatica ${transition.code} requiere condiciones`,
        );
      }
      if (
        ...
        throw new BadRequestException(
          `Rama else de ${transition.code} requiere estado destino`,
        );
      }
      if (
        transition.elseEffect === TRANSITION_EFFECTS.RUN_ACTIONS &&
        !transition.elseActions?.length
      ) {
        throw new BadRequestException(
          `Rama else de ${transition.code} requiere acciones`,
        );
      }

      const excludedStateIds = (transition.excludedStateRefs ?? []).map(
        (reference) => {
          const state = stateReferences.get(reference);
          if (!state) {
            throw new BadRequestException(
              `Transicion ${transition.code} excluye un estado inexistente`,
            );
          }
          return state.id;
        },
      ...
      ActionFactory.validateOrder(
        actions.filter(
          (action) =>
            action.transitionId === transitionId && action.branch === 'THEN',
        ),
      );
      ActionFactory.validateOrder(
        actions.filter(
          (action) =>
            action.transitionId === transitionId && action.branch === 'ELSE',
        ),
      );
      if (
        ...
        throw new BadRequestException(
          'Una transicion de acciones requiere al menos una accion',
        );
      }
      ...
      workflow: new Workflow({
        ...
        description:
          input.description === undefined
            ? (current?.workflow.description ?? null)
            : input.description,

...
@Injectable()
export class SaleOrdersRealtimeService {
  private readonly logger = new Logger(SaleOrdersRealtimeService.name);
  ...
  logStats() {
    this.logger.debug(
      `sale-orders realtime stats users=${this.connectedUsersCount()} sockets=${this.activeConnectionsCount()}`,
    );
  }
}

...
@Injectable()
export class WorkflowReactivityRealtimeService {
  private readonly logger = new Logger(WorkflowReactivityRealtimeService.name);
  ...
  logStats() {
    this.logger.debug(
      `workflow-reactivity realtime stats users=${this.connectedUsersCount()} sockets=${this.activeConnectionsCount()}`,
    );
  }
}

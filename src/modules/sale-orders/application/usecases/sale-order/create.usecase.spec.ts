...
describe('CreateSaleOrderUsecase', () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        CreateSaleOrderUsecase,
        {
          ...
          useValue: { runInTransaction: (work: any) => work(tx) },
        },
        {
          provide: PACK_REPOSITORY,
          useValue: { findByIdWithItems: jest.fn() },
        },
        {
          ...
          useValue: {
            reserveNext: jest
              .fn()
              .mockResolvedValue({ serie: 'PE', correlative: 7 }),
          },
        },
        {
        ...
        },
        {
          ...
          useValue: {
            findById: jest
              .fn()
              .mockResolvedValue({ id: 'workflow-1', isActive: true }),
          },
        },
        {
          provide: WORKFLOW_STATE_REPOSITORY,
          useValue: {
            findInitialByWorkflowId: jest
              .fn()
              .mockResolvedValue({ id: 'state-1' }),
          },
        },
  ...
  it('persists exactly the supplied pack snapshot without consulting the current pack', async () => {
    const tx = { id: 'create-tx' };
    const componentRepo = { bulkCreate: jest.fn().mockResolvedValue([]) };
    const packRepo = {
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            bulkCreate: jest
              .fn()
              .mockResolvedValue([{ id: 'item-1', quantity: 1 }]),
          },
        },
        {
          provide: SALE_ORDER_ITEM_COMPONENT_REPOSITORY,
    ...
    await expect(
      moduleRef.get(CreateSaleOrderUsecase).execute(
        {
          warehouseId: 'warehouse-1',
          clientId: 'client-1',
          workflowId: 'workflow-1',
  ...
  it('hydrates the current pack only when components are omitted', async () => {
    const tx = { id: 'create-tx' };
    const componentRepo = { bulkCreate: jest.fn().mockResolvedValue([]) };
    const packRepo = {
      findByIdWithItems: jest.fn().mockResolvedValue({
        pack: { id: 'pack-1', isActive: true },
        items: [
          {
            id: 'pack-item-1',
            skuId: 'sku-pack',
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            bulkCreate: jest
              .fn()
              .mockResolvedValue([{ id: 'item-1', quantity: 3 }]),
          },
        },
        {
          provide: SALE_ORDER_ITEM_COMPONENT_REPOSITORY,
    ...
    expect(packRepo.findByIdWithItems).toHaveBeenCalledWith('pack-1', tx);
    expect(componentRepo.bulkCreate).toHaveBeenCalledWith(
      [
        {

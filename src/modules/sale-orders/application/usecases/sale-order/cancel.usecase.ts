  ...
  async execute(input: { saleOrderId: string }) {
    return this.uow.runInTransaction(async (tx) => {
      ...
      if (order.deliveryStatus === DeliveryStatus.DELIVERED) {
        throw new BadRequestException("no se puede cancelar entregado");
      }
      ...
      await this.saleOrderRepo.updateStatuses(
        { saleOrderId: order.id, agendaStatus: AgendaStatus.CANCELED, deliveryStatus: DeliveryStatus.CANCELED },
        tx,
      );
      const deliveredOrdersCount = await this.saleOrderRepo.countSaleOrdersByClientId(order.clientId);
      if (deliveredOrdersCount === 0) {
        await this.clientRepo.update(
          {
            clientId: order.clientId,
            type: ClientType.LAGGING,
          },
          tx,
        );
      }

      return { saleOrderId: order.id, agendaStatus: AgendaStatus.CANCELED, deliveryStatus: DeliveryStatus.CANCELED };
    });
  }
}

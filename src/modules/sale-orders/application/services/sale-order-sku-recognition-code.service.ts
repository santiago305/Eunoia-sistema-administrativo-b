...
@Injectable()
export class SaleOrderSkuRecognitionCodeService {
  constructor(
    @InjectRepository(SaleOrderSkuRecognitionCodeEntity)
    private readonly repository: Repository<SaleOrderSkuRecognitionCodeEntity>,
  ...
  async update(id: string, input: SaveRecognitionCodeInput) {
    ...
    return this.dataSource.transaction(async (manager) => {
      const repository = manager.getRepository(SaleOrderSkuRecognitionCodeEntity);
      const current = await repository.findOne({ where: { id, isDeleted: false } });

...
describe('SaleOrderTypeormRepository', () => {
  it('loads tracking capabilities once for orders that share a workflow', async () => {
    ...
    const repositories = new Map<any, any>([
      ...
      [
        ...
        { createQueryBuilder: jest.fn().mockReturnValue(trackingActionQb) },
      ],
      [
        WorkflowEntity,
        {
          find: jest
            .fn()
            .mockResolvedValue([
              { id: 'workflow-1', name: 'Flujo', isActive: true },
            ]),
        },
      ],
    ...
    const manager = {
      getRepository: jest.fn(
        (entity) =>
          repositories.get(entity) ?? { find: jest.fn().mockResolvedValue([]) },
      ),
    };
    ...
    expect(trackingActionQb.andWhere).toHaveBeenCalledWith(
      'trackingWorkflow.isActive = true',
    );
    expect(trackingActionQb.andWhere).toHaveBeenCalledWith(
      'trackingTransition.isActive = true',
    );
    expect(trackingActionQb.andWhere).toHaveBeenCalledWith(
      'trackingTransition.isGlobal = true',
    );
    expect(trackingActionQb.andWhere).toHaveBeenCalledWith(
  ...
  it('builds SKUS and detail from item components in list items', async () => {
      ...
      find: jest.fn().mockResolvedValue([
        {
          ...
          skuId: 'sku-1',
          attributesSnapshot: [
            { code: 'variant', name: 'Variante', value: 'ANTI ACNE' },
          ],
          quantity: 1,
        ...
        },
        {
          ...
          skuId: 'sku-2',
          attributesSnapshot: [
            { code: 'variant', name: 'Variante', value: 'ANTI MANCHAS' },
          ],
          quantity: 1,
      ...
      [
        User,
        {
          find: jest
            .fn()
            .mockResolvedValue([
              { id: 'user-1', name: 'User', email: 'user@test.com' },
            ]),
        },
      ],
      [CompanyPaymentAccountEntity, { find: jest.fn().mockResolvedValue([]) }],
    ...
    expect(result.items[0].detail).toBe(
      'AMPOLLA ANTI ACNE x 1; AZUFRE ANTI MANCHAS x 1',
    );
  });
  ...
  it('marks preguide true idempotently', async () => {
    ...
    expect(update).toHaveBeenCalledWith({ id: 'order-1' }, { preguide: true });
  });
  ...
  it('marks prepared true idempotently', async () => {
    ...
    expect(update).toHaveBeenCalledWith({ id: 'order-1' }, { prepared: true });
  });
  ...
  it('unmarks preguide false idempotently', async () => {
    ...
    expect(update).toHaveBeenCalledWith({ id: 'order-1' }, { preguide: false });
  });
  ...
  it('unmarks prepared false idempotently', async () => {
    ...
    expect(update).toHaveBeenCalledWith({ id: 'order-1' }, { prepared: false });
  });
  ...
  it('updates reserveBool explicitly', async () => {
    ...
    await repository.setReserveBool({
      saleOrderId: 'order-1',
      reserveBool: true,
    });
  ...
  it('counts only active orders whose current state is final', async () => {
    ...
    await expect(
      repository.countSaleOrdersByClientId('client-1'),
    ).resolves.toBe(2);
  ...
  it('excludes final and cancelled states from automatic workflow candidates', async () => {
    ...
    await expect(repository.listIdsForAutomaticWorkflow()).resolves.toEqual([
      'order-1',
    ]);
    ...
    expect(qb.andWhere).toHaveBeenCalledWith(
      'upper(globalState.code) <> :cancelledCode',
      {
        cancelledCode: 'CANCELLED',
      },
    );
    expect(qb.addOrderBy).toHaveBeenCalledWith('so.id', 'ASC');
  });
  ...
  it('filters automatic workflow candidates by client id', async () => {
    ...
    await expect(
      repository.listIdsForAutomaticWorkflowByClientId('client-1'),
    ).resolves.toEqual(['order-1']);

    expect(qb.andWhere).toHaveBeenCalledWith('so.client_id = :clientId', {
      clientId: 'client-1',
    });
    expect(qb.andWhere).toHaveBeenCalledWith('currentState.isFinal = false');
    expect(qb.andWhere).toHaveBeenCalledWith(
  ...
  it('filters automatic workflow candidates by inventory stock event', async () => {
    ...
    expect(qb.andWhere).toHaveBeenCalledWith('so.warehouse_id = :warehouseId', {
      warehouseId: 'warehouse-1',
    });
    expect(qb.andWhere).toHaveBeenCalledWith(
      'inventoryStockItem.stock_item_id = :stockItemId',
      { stockItemId: 'stock-1' },
    );
    expect(qb.limit).toHaveBeenCalledWith(100);
  });
  ...
  it('loads the active main telephone when getting a sale order', async () => {
    ...
    const repository = new SaleOrderTypeormRepository({ manager } as any);
    const saleOrderRepo = manager.getRepository(
      require('../entities/sale-order.entity').SaleOrderEntity,
    );
    ...
    const clientRepo = manager.getRepository(
      require('src/modules/clients/adapters/out/persistence/typeorm/entities/client.entity')
        .ClientEntity,
    );
    ...
    const userRepo = manager.getRepository(
      require('src/modules/users/adapters/out/persistence/typeorm/entities/user.entity')
        .User,
    );
  ...
  it('returns enriched item components when getting a sale order', async () => {
    const { SaleOrderEntity } = require('../entities/sale-order.entity');
    const {
      SaleOrderItemEntity,
      ...
      getRepository: jest.fn((entity) => {
        ...
        if (entity === ProductCatalogStockItemEntity) {
          return {
            find: jest
              .fn()
              .mockResolvedValue([{ id: 'stock-1', skuId: 'sku-1' }]),
          };
        }
  ...
  it('applies bank account and client type filters to statistics', async () => {
    ...
    expect(baseQb.andWhere).toHaveBeenCalledWith(
      'client.type IN (:...stats_filter_1_value)',
      {
  ...
  it('applies creator and assigned user filters to list queries', async () => {
    ...
    expect(qb.andWhere).toHaveBeenCalledWith(
      'so.createdBy IN (:...filter_0_value)',
      {
        filter_0_value: ['creator-1', 'creator-2'],
      },
    );
    expect(qb.andWhere).toHaveBeenCalledWith(
      'so.assignedBy IN (:...filter_1_value)',
      {
      ...
      filters: [
        {
          field: 'workflowId',
          operator: 'in',
          values: ['workflow-1', 'workflow-2'],
        },
      ] as any,
    });

    expect(qb.andWhere).toHaveBeenCalledWith(
      'so.workflowId IN (:...filter_0_value)',
      {
  ...
  it('applies preguide and prepared status filters to list queries', async () => {
    ...
    expect(qb.andWhere).toHaveBeenCalledWith(
      'COALESCE(so.preguide, false) IN (:...filter_0_value)',
      {
        filter_0_value: [true],
      },
    );
    expect(qb.andWhere).toHaveBeenCalledWith(
      'COALESCE(so.prepared, false) IN (:...filter_1_value)',
      {
      ...
      filters: [
        {
          field: 'createdAt',
          operator: 'between',
          range: { start: '2026-07-09', end: '2026-07-09' },
        },
      ] as any,
    });
  ...
  it('applies lote text filters to list queries', async () => {
    ...
    await repository.list({
      filters: [{ field: 'lotes', operator: 'contains', value: '3' }] as any,
    });

    expect(qb.andWhere).toHaveBeenCalledWith(
      'CAST(so.lotes AS text) ILIKE :filter_0_value',
      {
        filter_0_value: '%3%',
      },
    );
  ...
  it('applies creator and assigned user filters to statistics', async () => {
    ...
    expect(baseQb.andWhere).toHaveBeenCalledWith(
      'so.createdBy IN (:...stats_filter_0_value)',
      {
        stats_filter_0_value: ['creator-1', 'creator-2'],
      },
    );
    expect(baseQb.andWhere).toHaveBeenCalledWith(
      'so.assignedBy NOT IN (:...stats_filter_1_value)',
      {
  ...
  it('applies preguide and prepared status filters to statistics', async () => {
    ...
    expect(baseQb.andWhere).toHaveBeenCalledWith(
      'COALESCE(so.preguide, false) IN (:...stats_filter_0_value)',
      {
        stats_filter_0_value: [false],
      },
    );
    expect(baseQb.andWhere).toHaveBeenCalledWith(
      'COALESCE(so.prepared, false) NOT IN (:...stats_filter_1_value)',
      {
  ...
  it('filters statistics to active sale orders and supports lote filters', async () => {
    ...
    await repository.statistics({
      filters: [{ field: 'lotes', operator: 'eq', value: '4' }] as any,
    });

    expect(baseQb.where).toHaveBeenCalledWith('so.isActive = true');
    expect(baseQb.andWhere).toHaveBeenCalledWith(
      'CAST(so.lotes AS text) = :stats_filter_0_value',
      {

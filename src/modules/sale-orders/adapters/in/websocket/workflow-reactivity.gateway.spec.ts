import { WorkflowReactivityGateway } from "./workflow-reactivity.gateway";
...
describe("WorkflowReactivityGateway", () => {
  ...
  it("responds to workflow-reactivity ping", () => {
    const gateway = new WorkflowReactivityGateway(realtimeService as any);
    const client = createClient("user-1");

    gateway.handlePing(client as any, { ts: 123 });

    expect(client.emit).toHaveBeenCalledWith("workflow-reactivity.pong", {

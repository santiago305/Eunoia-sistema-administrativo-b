...
import {
  EvaluateInventoryAlertsBatchDto,
  ListInventoryAlertSettingsDto,
} from '../dtos/list-inventory-alert-settings.dto';
...
export class InventoryAlertSettingsController {
  ...
  @Post('evaluate-batch')
  @HttpCode(200)
  evaluateBatch(@Body() dto: EvaluateInventoryAlertsBatchDto) {
    return this.predictiveAlerts.evaluateBatch(dto.items);
  }
  ...
  update(
    ...
    return this.predictiveAlerts
      .evaluate(stockItemId, dto.warehouseId)
      .then(({ policy }) =>

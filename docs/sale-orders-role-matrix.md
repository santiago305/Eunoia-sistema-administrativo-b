...
| Seguimiento de Envíos | Consulta + `sale_orders.preguide.update`, `sale_orders.prepared.update` | No incluye edición financiera ni gestión de workflows. |
...
- `sale_orders.preguide.update` y `sale_orders.prepared.update` controlan cambios individuales y masivos, incluyendo `true` y `false`.
...
La fuente única de códigos es `src/modules/sale-orders/application/constants/sale-order-permissions.ts`. El catálogo contiene 39 permisos: uno de página y 38 de acciones.
...
4. Verificar HTTP, WebSocket y acciones masivas con la misma matriz.
